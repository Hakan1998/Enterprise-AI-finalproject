...
def evaluate_content_based(raw_test_data: pd.DataFrame, cosine_sim: Any, k: int = 10) -> Tuple[float, float]:
    ...
    for idx, row in raw_test_data.iterrows():
        ...
        sim_scores = sim_scores[1:k+1]  # Skip the first score as it is the movie itself

        recommended_movie_ids = [raw_test_data.iloc[i[0]]['id'] for i in sim_scores if i[0] != idx]
        total_recommended += len(recommended_movie_ids)

        if actual_rating >= 3.5:  # Assuming ratings of 3.5 and above are positive
            total_relevant += 1
